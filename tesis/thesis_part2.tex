...
\begin{figure*}[t]
  ...
  ($f = 0.9994$ at $\ttr = 45$), whereas the STA-global $\sin^2$ (green
  ...
  STA-global $\sin^2$ ($\ttr = 75$) protocols, respectively.}
...
\begin{figure*}[t]
  ...
  profile (standard $\sin^2$, STA $\alpha = 2$, STA-global $\sin^2$).
  ...
  strongly asymmetric position ($j_{\mathrm{DW}} = 7$), respectively. At every
...
\begin{figure}[t]
  ...
  the three-level model no longer represents the chain. Dotted line: fault-tolerance
  threshold $f_0=0.995$.}
...
exceeds the fault-tolerance threshold $f_0=0.995$, recovering essentially
...
\caption{Validation of CTAP on the full $L=21$ SSH Hamiltonian~(1).
  ...
  and clear the fault-tolerance threshold $f_0=0.995$ (dashed) at every position,
...
Hamiltonian always possesses a zero-energy dark state $|D_0(t)\rangle =
\cos\Theta(t)|L\rangle - \sin\Theta(t)|R\rangle$---a direct consequence of chiral
symmetry---that mediates $|L\rangle\to|R\rangle$ transfer without populating the
soliton state. Integrating the full $L=21$ chain, the counter-intuitive pulse
sequence recovers $f>0.999$ at wall positions where the Rabi fidelity collapses
to $\lesssim10^{-2}$, with bulk leakage below $10^{-7}$ and an effective
three-level reduction accurate to better than $0.025\%$. The only residual
infidelity is a removable pulse boundary-condition effect, $f\le\cos^2\Theta_0$;
the sole genuine limit ($\rho\gtrsim10^2$) is geometric---the protected gap
closing---not a failure of the protocol.
...
\textbf{8.~Multi-domain construction gives exponential acceleration with a
collective dark state.} Subdividing the chain into $D$ domains at fixed total
length shrinks the bottleneck segment and reduces the transfer time by more than
four orders of magnitude ($\sim\!1.3\times10^4$ at $D=4$). A single
counter-intuitive collective pulse sequence transfers across the resulting
four-domain chain with fidelity $F=0.998$, keeping the inner solitons dark and
using nearest-neighbour couplings only---so the speed-up is obtained without
sacrificing chiral symmetry.

\textbf{9.~Chiral symmetry, not topology alone, governs robustness.} Chiral
symmetry pins the edge states at $E=0$: off-diagonal (hopping) disorder leaves
the zero mode at machine precision and the CTAP fidelity flat at $\approx0.99$,
whereas on-site disorder and next-nearest-neighbour hoppings break the symmetry
and detach the mode ($\langle|E_0|\rangle\approx0.95\,\kappa$ for NNN), degrading
the transfer. Because CTAP uses only nearest-neighbour couplings it inherits this
protection, while the NNN-based CD protocol forfeits it: the control protocol,
not the topological phase by itself, determines whether the transfer is robust.
CTAP is also $\sim\!8\times$ more tolerant to domain-wall misplacement than the
static Rabi protocol.

\textbf{10.~Even-state parity constraint limits two-domain-wall CTAP.}
