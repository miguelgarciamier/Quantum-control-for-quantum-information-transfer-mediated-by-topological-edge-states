...
time-modulated (``dynamic'') pulse sequences considered in Secs.~\ref{sec:sta}
and~\ref{sec:ctap}.
...
\begin{figure*}[!t]
  ...
  \caption{Combined effect of asymmetry and STA pulses ($L = 21$).
...
multi-domain construction of Sec.~\ref{sec:ctap_twodw} removes.
...
All Gaussian pulses [Eqs.~(\ref{eq:ctap_jsr})--(\ref{eq:ctap_jls})] share
$\sigma=0.18\,T$, but the two main scripts differ in the pulse centers:
the full-chain figure (\texttt{fig4\_fullchain\_ctap.py},
\texttt{pulse\_params()}) sets $t_R = \tfrac{1}{2}T - \sigma$ and
$t_L = \tfrac{1}{2}T + \sigma$, giving $t_L - t_R = 2\sigma$; the effective
three-level comparison (\texttt{ctap\_asymmetric.py}) uses $t_R = T/3$ and
$t_L = 2T/3$, giving $t_L - t_R = T/3 \approx 1.85\,\sigma$.
In both cases $t_R < t_L$, enforcing the counter-intuitive ordering.
The total protocol time is set by $T = \tau_{\mathrm{ad}}/J_{\mathrm{bott}}$
with $\tau_{\mathrm{ad}}=180$ and $J_{\mathrm{bott}}=\min(J_{\max}^L,J_{\max}^R)$
(the bottleneck effective coupling at $v = v_{\max}=0.5$), guaranteeing that
the adiabaticity condition [Eq.~(\ref{eq:adiabaticity})] is satisfied.
Full-chain dynamics are integrated with \texttt{scipy.integrate.solve\_ivp}
(RK45, $\mathrm{rtol}=10^{-6}$, $\mathrm{atol}=10^{-8}$, 400 output points);
effective three-level dynamics use DOP853 ($\mathrm{rtol}=10^{-11}$,
