...
\usepackage{bm}
\usepackage{hyperref}
\usepackage{xurl}  % permite cortar URLs largas en cualquier carácter (evita el desbordamiento de columna)
\usepackage{xcolor}
\usepackage{braket}
\usepackage{booktabs}
\usepackage{siunitx}
...
% --- Float placement tuning (avoid figures stranded alone on a page) ---
...
% ============================================================================
% 1. PORTADA PERSONALIZADA (TITLEPAGE)
% ============================================================================
\begin{titlepage}
  \centering
  \vspace*{0.5cm}
  ...
  \begin{minipage}{\textwidth}
    \centering
    \includegraphics[height=1.7cm]{logo.jpg}\hfill
    \includegraphics[height=1.7cm]{Logotipo_del_CSIC.pdf}
  \end{minipage}\\[3.5cm]
\vspace{3.5cm}

  % Bloque central: Logo del máster + Nombre del Máster
  \begin{minipage}{0.8\textwidth}
    \centering
    % LOGO DEL MÁSTER (Ajusta el ancho y el nombre del archivo)
    \begin{minipage}{\textwidth}
        \centering
        \includegraphics[width=\linewidth]{master_logo.png} 
    \end{minipage}%
    \hspace{0.5cm}%
  \end{minipage}
  ...
  % Título
  \vspace{0.3cm}
  {\LARGE\bfseries Quantum control for quantum information transfer mediated by topological edge states \par}
  
  \vspace{2.5cm}

  % Autor y Directores
  {\large Author: Miguel Garc\'ia-Mier Garc\'ia \par}
  \vspace{0.2cm}
  {\large Supervisors: Prof. Gloria Platero \& Dr. Yue Ban \par}
...
% ============================================================================
% 2. ÍNDICE Y CONFIGURACIÓN DE PÁGINAS
% ============================================================================
\clearpage
%\pagenumbering{roman} % Números romanos para el índice (i, ii, iii...)

\onecolumngrid % Una sola columna para que el índice ocupe todo el ancho
\tableofcontents
\thispagestyle{empty}
\clearpage
\twocolumngrid % Restauramos las dos columnas para el documento

\pagenumbering{arabic} % Reiniciamos la numeración en 1 para la introducción
% ============================================================================


% ============================================================================
% 3. CABECERA TIPO "PAPER" DE LA APS (METADATOS Y ABSTRACT)
% ============================================================================
% Título en texto plano (esto arregla el error de hyperref)
\title{Quantum control for quantum information transfer mediated by topological edge states}

\author{Miguel Garc\'ia-Mier Garc\'ia}
\affiliation{%
  MSc in Quantum Technologies, Universidad Internacional Men\'endez Pelayo, Spain
}

% Añadimos a las tutoras también en la cabecera del artículo (ajusta la afiliación si hace falta)
\author{Supervisors: Gloria Platero \& Yue Ban}
\affiliation{Instituto de Ciencia de Materiales de Madrid (ICMM-CSIC)}
...
the static Rabi transfer protocol of Ref.~\cite{Zurita2023} rather than of topological protection itself.
We further explore shortcut-to-adiabaticity (STA)-inspired ramp shaping---pulse
profiles designed to suppress diabatic excitations and emulate adiabatic
evolution in finite time---and find that
...
Third, motivated by the CTAP (Coherent Tunneling Adiabatic Passage) formalism,
we replace the static Rabi protocol by a dynamic counter-intuitive pulse sequence
that exploits the topological dark state of the effective three-level
Hamiltonian. Numerical integration of the full $L=21$ chain recovers fidelities
$f>0.999$ at wall positions where the Rabi protocol collapses, with bulk leakage
below $10^{-7}$ and an effective three-level reduction that is quantitatively
exact; the residual infidelity is a removable pulse boundary-condition effect,
not a topological bound. A collective version of the protocol transfers across a
multi-domain chain with fidelity $F=0.998$ while shortening the transfer time by
more than four orders of magnitude. Crucially, because CTAP uses only
nearest-neighbor couplings it inherits the chiral protection that pins the edge
states against off-diagonal disorder, whereas the next-nearest-neighbor driving
of the CD protocol breaks that symmetry: the control protocol, not the topology
alone, determines whether the transfer is robust. We also discuss the even-state
parity constraint limiting the two-domain-wall case.
\end{abstract}
...
\begin{enumerate}
  ...
  \item Can shortcut-to-adiabaticity (STA)-inspired ramp shaping (pulse-profile
    optimization of the preparation/readout ramps)~\cite{Demirplak2003, Berry2009, GueryOdelin2019} improve the
...
We refer to this scheme as the \emph{static Rabi protocol}: ``static'' because
the effective couplings $\JLS,\JSR$ are held constant during the transfer phase,
and ``Rabi'' because the population then undergoes a Rabi-like oscillation
$\ketL \to \ketS \to \ketR$. This naming distinguishes it from the
time-modulated (``dynamic'') pulse sequences considered in
Sec.~\ref{sec:ctap}.
...
$\tprep \gg \tau = 2/\Delta$, where $\Delta$ [Eq.~(\ref{eq:gap})] is the instantaneous spectral gap.
...
reported in Ref.~\cite{Zurita2023}. The peak fidelity $f = 0.999$ exceeds $f_0 = 0.995$, which we adopt throughout
as a stringent high-fidelity benchmark, motivated by values often discussed in
the context of fault-tolerant quantum error correction~\cite{Knill2005}.
...
\begin{table}[b]
  ...
  $L = 21$ chain ($v = 0.5$, $w = 1.0$). $E_{\pm} = \pm\sqrt{\JLS^2+\JSR^2}$ are
  the energies of the two split levels of the effective three-site chain
  [Eq.~(\ref{eq:heff})], the central level remaining pinned at $E = 0$.
  ``Min.~overlap'' is the smallest singular value of the overlap matrix between
  the analytical states [Eqs.~(\ref{eq:ketL})--(\ref{eq:ketR})] and the numerical
  eigenstates (unity = perfect agreement). $\JLS/\JSR$ is the effective coupling
  ratio that sets the Rabi bound of Eq.~(\ref{eq:fmax}).}
...
\begin{figure*}[!t]
  ...
  \caption{Spatial profiles of the three protected states $\ketL$, $\ketS$,
  $\ketR$ for different domain wall positions in an $L = 21$ chain:
  \textbf{(a)} $j_{\mathrm{DW}} = 11$ (quasi-center),
  \textbf{(b)} $j_{\mathrm{DW}} = 7$ (one-third),
  \textbf{(c)} $j_{\mathrm{DW}} = 5$ (one-quarter). As the wall moves toward the
  left edge, $\ketL$ is compressed into the shorter domain while $\ketR$ spreads
  over the longer one; the exponential localization and sublattice polarization
  (alternating signs) are preserved in all cases.}
...
predicted by Eq.~(\ref{eq:coupling_ratio}), with the numerical values
listed in Table~\ref{tab:couplings}.
...
\begin{table}[b]
  ...
  \begin{tabular}{ccccl}
    ...
    \colrule
    11 & 0.0105 & 0.0210 & 0.50 & $(v/w)^1 = 0.50$ \\
    7  & 0.0423 & 0.0053 & 8.0  & $(w/v)^3 = 8.0$ \\
    5  & 0.0862 & 0.0027 & 32.0 & $(w/v)^5 = 32.0$ \\
  \end{tabular}
  \end{ruledtabular}
\end{table}
...
\begin{table}[b]
  \caption{Optimal transfer fidelity for the asymmetric domain wall positions
  ($L = 21$, standard $\sin^2$ pulse, $\tprep = 15$). $\ttr^{\mathrm{opt}}$ is the
  protocol time of the first fidelity maximum from a scan over
  $\ttr \in [30,600]$, and $f_{\max}$ the corresponding peak fidelity. The peaks
  track the analytical Rabi bound of Eq.~(\ref{eq:fmax}), set by the coupling
  ratio $\JLS/\JSR$ of Table~\ref{tab:couplings}.}
...
\begin{figure*}[!t]
  ...
  \caption{Transfer dynamics for the three asymmetric domain wall configurations
  ($L = 21$, standard $\sin^2$ pulse, $\tprep = 15$):
  \textbf{(a)} $j_{\mathrm{DW}} = 11$, \textbf{(b)} $j_{\mathrm{DW}} = 7$,
  \textbf{(c)} $j_{\mathrm{DW}} = 5$, each at its optimal $\ttr$
  (Table~\ref{tab:asym_fidelity}). Each panel shows the shaded ramp $v(t)$ (left
  axis) and the occupations of the three localized sites---site $0$ ($\ketL$),
  the domain wall ($\ketS$) and site $L-1$ ($\ketR$) (right axis). As the wall
  moves off-center, the coupling asymmetry $\JLS \neq \JSR$ caps the population
  that can be swapped into $\ketR$: the transfer is essentially complete in (a)
  but increasingly incomplete in (b) and (c), where the excitation remains
  trapped in $\ketL$ and $\ketS$.}
...
\begin{figure*}[!t]
  ...
  \caption{Transfer fidelity for the three domain wall positions in the
  $L = 21$ chain. \textbf{(a)} Fidelity $f$ vs.~the protocol time $\ttr$ for the
  quasi-center ($\sim 1/2$), one-third ($\sim 1/3$) and one-quarter ($\sim 1/4$)
  wall positions; dots mark the first maximum. \textbf{(b)} The corresponding
  peak fidelity (left axis, bars) and optimal $\ttr$ (right axis) for
  $j_{\mathrm{DW}} = 11, 7, 5$. The first fidelity maximum decreases
  exponentially with the displacement from the center, consistent with the
  coupling asymmetry of Eq.~(\ref{eq:coupling_ratio}).}
...
\begin{itemize}
  ...
  \item $L = 15$: peak fidelity $f = 0.994$ at $j_{\mathrm{DW}} = 7$ (center),
    ...
    $j_{\mathrm{DW}} = 3$.
\end{itemize}
The slightly lower centered value for $L=11$ ($f=0.96$) relative to the
optimized reproduction of Sec.~\ref{sec:results_symmetric} ($f=0.999$)
reflects the coarser $\Delta t_{\mathrm{tr}}=10$ scan grid used here: the
true optimum near $t_{\mathrm{tr}}\approx 45$ falls midway between grid
points, so the scan reads $f$ at $t_{\mathrm{tr}}\in\{40,50\}$ rather
than at the exact peak.
...
The exact counterdiabatic Hamiltonian~\cite{Berry2009}
\begin{equation}
...
The variants below are \emph{STA-inspired ramp-shaping} strategies---pulse-profile
optimizations of the preparation and readout ramps---rather than exact STA
protocols. As such they act only on the ramp phases and, as we show below, cannot
overcome the coupling-asymmetry bound, since they do not dynamically control the
ratio $\JLS(t)/\JSR(t)$ that governs the transfer phase.
...
\begin{enumerate}
  ...
  \item \textbf{Ramp-shaping ($\alpha$)}: A modified ramp $v(t) = v_{\mathrm{tr}}
  ...
  \item \textbf{Global $\sin^2$ ramp}: A single smooth profile over the entire
    protocol:
    \begin{equation}
...
\begin{table}[b]
  ...
  \begin{tabular}{lccc}
    ...
    Ramp-shaping ($\alpha = 2$) & 10 & 41  & 0.9706 \\
    Ramp-shaping ($\alpha = 3$) & 10 & 43  & 0.9398 \\
    Global $\sin^2$ ramp    & -- & 75  & 0.99995 \\
    Linear                  & 15 & 97  & 0.9924 \\
  \end{tabular}
  \end{ruledtabular}
\end{table}
...
\begin{figure*}[!t]
  ...
  \caption{Comparison of the pulse profiles for the symmetric chain
  ($L = 11$, $\ell = 4$). \textbf{(a)} The ramp shapes $v(t)$ and
  \textbf{(b)} the resulting fidelity $f$ vs.~$\ttr$ for all profiles (shared
  legend); dots mark each optimum. The standard $\sin^2$ pulse with
  $\tprep = 15$ (solid blue) gives the best time--fidelity trade-off
  ($f = 0.9994$ at $\ttr = 45$), whereas the Global $\sin^2$ ramp (green
  dash-dot) reaches the highest absolute fidelity ($f = 0.99995$) at the cost of
  $\ttr = 75$. \textbf{(c)},\textbf{(d)} Site-resolved occupation
  $\langle n_j\rangle$ vs.~time for the standard $\sin^2$ ($\ttr = 45$) and the
  Global $\sin^2$ ramp ($\ttr = 75$) protocols, respectively.}
...
\begin{itemize}
  ...
  \item The Global $\sin^2$ ramp achieves the highest absolute fidelity
    ($f = 0.99995$) but at the cost of a 67\% longer protocol time
    ($\ttr = 75$ vs.~45).
  \item The Ramp-shaping ($\alpha$) pulses with reduced $\tprep = 10$ \emph{do not}
...
preparation time $\tprep$ for the standard and Ramp-shaping ($\alpha$) pulses. The
...
\subsection{Results: asymmetric chains with ramp shaping}
\label{sec:sta_asymmetric}

As a controlled negative test, we ask whether ramp shaping can compensate for
the fidelity degradation caused by asymmetric domain-wall placement. We test the standard,
Ramp-shaping ($\alpha = 2$), and Global $\sin^2$ ramp on the three asymmetric
configurations ($L = 21$, $j_{\mathrm{DW}} = 11, 7, 5$).

\begin{table}[b]
  \caption{Pulse comparison for asymmetric configurations ($L = 21$). No pulse
  protocol significantly improves the fidelity. This confirms that the
  limitation is not due to diabatic ramp excitations, but to the coupling
  asymmetry in the static Rabi transfer phase.}
  \label{tab:sta_asym}
  \begin{ruledtabular}
  \begin{tabular}{lccc}
    $j_{\mathrm{DW}}$ & Standard & Ramp-shaping ($\alpha=2$) & Global $\sin^2$ \\
    \colrule
...
\begin{figure*}[!t]
  ...
  \caption{Combined effect of asymmetry and ramp-shaping pulses ($L = 21$).
  \textbf{(a)} Optimal protocol time $\ttr^{\mathrm{opt}}$ and peak fidelity for
  every combination of wall position ($j_{\mathrm{DW}} = 11, 7, 5$) and pulse
  profile (standard $\sin^2$, Ramp-shaping ($\alpha = 2$), Global $\sin^2$ ramp).
  \textbf{(b)},\textbf{(c)} Fidelity vs.~$\ttr$ for the three pulse profiles at
  the quasi-center ($j_{\mathrm{DW}} = 11$, peak $f \approx 0.64$) and at a
  strongly asymmetric position ($j_{\mathrm{DW}} = 5$), respectively. At every
  position the three pulse variants are essentially indistinguishable and none
  approaches $f_0 = 0.995$: pulse shaping acts only on the ramps and cannot
  overcome the coupling asymmetry that governs the transfer phase.}
...
preparation and readout fidelity; \textbf{the transfer phase itself is governed
by the effective couplings $\JLS$ and $\JSR$, which are fixed by the domain-wall
geometry [Eq.~(\ref{eq:coupling_ratio})] and not by the ramp profile.} Reshaping
the ramp therefore cannot alter the bound $f_{\max}$ of Eq.~(\ref{eq:fmax}): only
a protocol that varies the coupling ratio $\JLS(t)/\JSR(t)$ in time---such as the
CTAP scheme of Sec.~\ref{sec:ctap}---can overcome the asymmetry.
...
diagram---a Thouless-pump-like protocol~\cite{Thouless1983} whose zero-energy eigenstate migrates from one boundary
...
it consists of next-nearest-neighbor (NNN) hoppings between sites of the \emph{same} sublattice. Related schemes that accelerate and stabilize SSH edge-state transfer with engineered NNN couplings have also been explored~\cite{DAngelis2020}.
...
becomes less accurate as the chain length increases, since higher-order CD corrections---which
...
the time-critical ramp phases---precisely the regime where the ramp-shaping
variants of Sec.~\ref{sec:sta} were shown to provide only marginal improvements.
...
In this section we show, following the CTAP (Coherent Tunneling Adiabatic
...
Adiabatic Passage)~\cite{Vitanov2017, GueryOdelin2019}. The counter-intuitive aspect is
...
\end{align}
where $t_R < t_L$ (the right pulse fires first) and the pulse separation
$t_L - t_R$ is a tunable parameter of order $\sigma$, chosen per configuration
to satisfy the adiabaticity condition~(\ref{eq:adiabaticity}); the specific
values used in each figure are listed in Sec.~\ref{sec:ctap_implementation}.
...
the CD term can be approximated using local operators via the variational CD
...
\begin{figure}[tb]
  ...
  $L=21$ ($v=0.5$, $w=1.0$), from numerical integration of the effective
  three-level Hamiltonian~(21). The static Rabi protocol (red) peaks at
  $f\approx0.64$ near the quasi-center and collapses to $f\lesssim0.004$ for
  $j_{\mathrm{DW}}\le5$. With well-conditioned pulses [Eqs.~(25)--(26); peaks at
  $T/4$ and $3T/4$, $\sigma=0.12T$], bare CTAP (blue) and CD-assisted CTAP (teal)
  both sustain $f\gtrsim0.99$ at every position where the three-level description
  holds, and coincide because the counterdiabatic term changes the transfer
  \emph{time}, not the fidelity. Shaded bands ($j_{\mathrm{DW}}\in\{1,3,17,19\}$,
  $\rho\gtrsim10^2$) mark where the weaker coupling closes the protected gap and
  the three-level model no longer represents the chain. Dotted line: high-fidelity
  benchmark $f_0=0.995$.}
...
$\rho$ and pins down the origin of the residual infidelity. At fixed pulse
separation the converged bare-CTAP fidelity does not remain at unity but
follows the analytic ceiling $f=\cos^2\Theta_0(\rho)$ derived below
[Eq.~(\ref{eq:ceiling})]; the ceiling falls with $\rho$ only because the weaker
pulse amplitude $1/\rho$ leaves a larger residual overlap $\Theta_0$ at the
protocol endpoints. Re-conditioning the pulses (wider separation, narrower
width) drives $\Theta_0\to0$ and restores $f\approx1$ across the entire range.
The genuine physical limit appears only for $\rho\gtrsim10^2$, where the weaker
coupling $J_{\min}\sim1/\rho$ shrinks the instantaneous gap
$\sqrt{J_{LS}^2+J_{SR}^2}$ until the three protected levels become
quasi-degenerate and the effective three-level description---not the CTAP
mechanism itself---breaks down. CTAP thus removes the asymmetry bottleneck of
the static Rabi protocol across the entire experimentally relevant range; it
cannot manufacture coupling where the geometry forbids it.

\begin{figure}[tb]
  \centering
  \includegraphics[width=\columnwidth]{fig16_ctap_asymmetry.pdf}
  \caption{Transfer fidelity versus the peak-coupling ratio
  $\rho=J^{L}_{\max}/J^{R}_{\max}$ for the effective three-level system,
  identifying the residual ceiling as a pulse boundary-condition effect. At the
  original pulse separation, the converged bare-CTAP fidelity (blue, $T=400\rho$)
  tracks the analytic ceiling $\cos^2\Theta_0(\rho)$ [dashed; Eq.~(29)]. Re-conditioned
  pulses (teal; peaks at $T/4$ and $3T/4$, $\sigma=0.12T$) drive $\Theta_0\to0$ and
  keep $f\approx1$ across the whole range. Beyond $\rho\gtrsim10^2$ (shaded) the
  protected gap of the real chain closes and the three-level description ceases to
  apply. Dotted line: $f_0=0.995$.}
  \label{fig:ctap_asymmetry}
\end{figure}

\subsection{Validation on the full SSH chain}
\label{sec:fullchain}

Sections~\ref{sec:ctap_darkstate}--\ref{sec:ctap_numerics} established the CTAP mechanism
on the effective three-level Hamiltonian~(21). That description, however,
retains only the three protected states and discards the remaining $L-3=18$
bulk levels of the physical chain. A legitimate concern is whether the
counter-intuitive drive, when applied to the \emph{full} $L=21$
Hamiltonian~(1), still follows the dark state or instead leaks into the bulk
continuum that the reduced model cannot see. We therefore integrate the
complete time-dependent Schr\"odinger equation for the three representative
wall positions, implementing the two couplings through the independent domain
controls $v_L(t)$ and $v_R(t)$ of Sec.~\ref{sec:ctap_pulse} [pulse
parameters $\sigma=0.15\,T$, $t_L-t_R=0.20\,T$ for $j_{\rm DW}=11,7$ and
$\sigma=0.18\,T$, $t_L-t_R=0.36\,T$ for $j_{\rm DW}=5$].

The outcome is summarized in Table~\ref{tab:fullchain} and
Fig.~\ref{fig:fullchain}. At every wall position the full-chain CTAP fidelity
exceeds the high-fidelity benchmark $f_0=0.995$, recovering essentially
perfect transfer precisely where the static Rabi protocol is useless:
$f=0.9998$ at $j_{\rm DW}=11$ (against the Rabi optimum $0.637$), $f=0.9999$ at
$j_{\rm DW}=7$ (against $0.061$), and $f=0.9995$ at $j_{\rm DW}=5$ (against
$0.004$)---an improvement of up to two and a half orders of magnitude at the
strongest asymmetry.

Two features confirm that this is genuine dark-state transport rather than a
fortuitous overlap. First, the population outside the three-state protected
subspace never exceeds $\sim\!10^{-7}$ throughout the protocol: the chiral gap
isolates the dark state from the bulk, so the counter-intuitive drive does not
excite the continuum. Second, the full-chain fidelity reproduces the
effective-model prediction to better than $0.025\%$, so the $3\times3$
reduction~(21) underlying Figs.~\ref{fig:ctap_comparison}--\ref{fig:ctap_asymmetry}
is not a phenomenological approximation but a quantitatively exact projection of
the full dynamics. In particular, the $\cos^2\Theta_0$ ceiling of
Sec.~\ref{sec:ctap_ceiling} is a property of the real chain, not of the toy model.

The price of asymmetry resilience is paid in time, not fidelity. Adiabaticity
through the weaker effective coupling $J_{\min}\sim v\,(v/w)^{\ell_{\max}/2}$
requires a protocol duration $T\gtrsim J_{\min}^{-1}$ that grows exponentially
with the longer (bottleneck) domain: $T$ increases from $\approx2.6\times10^{4}$
at $j_{\rm DW}=11$ to $\approx6.1\times10^{4}$ at $j_{\rm DW}=5$. This
exponential cost is intrinsic to a single domain wall and is exactly what the
multi-domain construction of Sec.~\ref{sec:multidomain} removes.

\begin{table}[!t]
\caption{Full-chain CTAP on the $L=21$ SSH Hamiltonian~(1) for the three
representative wall positions. $f_{\rm Rabi}$ is the static-Rabi optimum
(Table~III); $f_{\rm CTAP}$ is the converged counter-intuitive-pulse fidelity;
the last column is the maximum population leaked outside the three protected
states during the protocol.}
\label{tab:fullchain}
\begin{ruledtabular}
\begin{tabular}{cccccc}
$j_{\rm DW}$ & $(\ell_1,\ell_2)$ & $\rho$ & $f_{\rm Rabi}$ & $f_{\rm CTAP}$ & bulk leakage \\
\colrule
11 & (11,\,9)  & 2  & 0.637 & 0.9998 & $\sim\!10^{-7}$ \\
7  & (7,\,13)  & 8  & 0.061 & 0.9999 & $\sim\!10^{-7}$ \\
5  & (5,\,15)  & 32 & 0.004 & 0.9995 & $\sim\!10^{-7}$ \\
\end{tabular}
\end{ruledtabular}
\end{table}
...
\begin{figure*}[!t]
\centering
\includegraphics[width=\textwidth]{figures/fig_fullchain_ctap.pdf}
\caption{Validation of CTAP on the full $L=21$ SSH Hamiltonian~(1).
  \textbf{(a)} Counter-intuitive drive in the independent domain controls:
  the right-domain hopping $v_R(t)$ (Stokes, blue) peaks at $t_R\approx0.32\,T$
  and fires before the left-domain hopping $v_L(t)$ (pump, red,
  $t_L\approx0.68\,T$), rotating the mixing angle $\Theta(t)$ from $0$ to
  $\pi/2$ (panels (a)--(c) shown for $j_{\rm DW}=5$, the strongest asymmetry).
  \textbf{(b)} Population dynamics from the complete time-dependent
  Schr\"odinger equation: $P_{\mathcal L}$ (red) is transferred to
  $P_{\mathcal R}$ (teal) while the soliton population $P_{\mathcal S}$ (orange)
  stays $\approx0$. Inset (log scale): the population leaked outside the three
  protected states never exceeds $\sim\!2\times10^{-7}$, confirming that the
  chiral gap isolates the dark state from the bulk continuum.
  \textbf{(c)} Site-resolved density $|\psi_j(t)|^2$: the excitation flows from
  site $0$ to site $20$ without accumulating on the domain-wall site (DW,
  marked)---the spatial signature of dark-state transport.
  \textbf{(d)} Transfer fidelity at the three representative wall positions for
  the static Rabi optimum (Rabi), the effective three-level CTAP
  ($\mathrm{CTAP}_{\rm eff}$) and the full-chain CTAP ($\mathrm{CTAP}_{\rm full}$).
  The full-chain values reproduce the effective model to better than $0.025\%$
  and clear the high-fidelity benchmark $f_0=0.995$ (dashed) at every position,
  whereas the Rabi fidelity collapses away from the center. Annotated transfer
  times ($T\approx2.56\times10^{4},\,3.07\times10^{4},\,6.14\times10^{4}$) grow
  exponentially with the bottleneck domain.}
\label{fig:fullchain}
\end{figure*}

\subsection{The fidelity ceiling as a boundary-condition effect}
\label{sec:ctap_ceiling}
...
\subsection{Multi-domain acceleration and collective CTAP}
\label{sec:multidomain}

The full-chain results of Sec.~\ref{sec:fullchain} show that single-domain-wall
CTAP is asymmetry-resilient but pays an exponential price in time: the duration
is set by the weakest effective coupling, $T\gtrsim J_{\min}^{-1}$ with
$J_{\min}\sim v\,(v/w)^{\ell_{\max}/2}$, and therefore grows exponentially with
the bottleneck domain. The multi-domain construction of
Zurita~\emph{et al.}~\cite{Zurita2023} removes this cost. Subdividing a chain of
fixed total length into $D$ domains introduces $D-1$ internal domain walls, each
hosting a soliton, so the protected subspace grows to the $D+1$ states
$\{|L\rangle,|S_1\rangle,\dots,|S_{D-1}\rangle,|R\rangle\}$ arranged as a
$(D+1)$-site effective tight-binding chain. The bottleneck segment shrinks to
$\ell_s\simeq L/D$, and since each effective coupling scales as $(v/w)^{\ell_s/2}$,
the transfer time collapses exponentially as $D$ increases.

Figure~\ref{fig:multidomain}(a) quantifies this acceleration for a chain of fixed
length ($L\simeq45$). As $D$ grows from $1$ to $4$ the bottleneck segment
contracts from $\ell_s=43$ to $\ell_s=11$, and the estimated transfer time
relative to the single-segment reference drops by $T_1/T_D=916$, $4367$, and
$13134$ for $D=2,3,4$---almost four orders of magnitude at $D=4$. The per-segment
scaling $J_{\rm bott}\sim(v/w)^{\ell_s/2}$ is reproduced empirically across the
whole range.

The speed-up is only useful if the dark-state mechanism survives the
proliferation of intermediate solitons. We verify this by driving the full $D=4$
chain ($L=45$, five protected states) with a single counter-intuitive
\emph{collective} pulse sequence and integrating the complete time-dependent
Schr\"odinger equation. The collective pulses are shaped directly in the effective couplings $J(t)$
rather than in the bare hopping $v(t)$: a Gaussian profile in $v$ collapses the
instantaneous protected gap by up to $\sim\!500\times$ at the pulse crossover for
the longer segments, whereas shaping in $J$ keeps the gap open and the evolution
adiabatic. Figure~\ref{fig:multidomain}(b) shows the outcome:
population flows from $|L\rangle$ to $|R\rangle$ with fidelity $F=0.9976$, while
the inner solitons stay essentially dark---$|S_1\rangle$ and $|S_3\rangle$ remain
at $\approx0$ throughout and $|S_2\rangle$ is only transiently populated (peak
$\approx0.24$ at $t/T=\tfrac12$), exactly as the odd-site collective dark state
predicts. The protocol uses nearest-neighbor couplings only, so it inherits the
chiral protection of the single-wall scheme, and it completes $14.3\times$ faster
than the $D=2$ transfer over the same chain. The curves start and end near
$0.93$ rather than unity because the injected localized site overlaps the
protected $|L\rangle$ state at $0.93$; this is an initialization overlap, not
leakage.

The collective dark state exists because the $D=4$ configuration has an \emph{odd}
number of effective sites ($D+1=5$). Even-site configurations admit no single
isolated dark state and require the straddling generalization analyzed in
Sec.~\ref{sec:ctap_twodw}.

\begin{figure*}[!t]
\centering
\includegraphics[width=\textwidth]{figures/fig_multidomain_speedup.pdf}
\caption{Multi-domain acceleration of CTAP. (a) Speed-up $T_1/T_D$ of the
estimated transfer time as a chain of fixed total length is subdivided into $D$
domains (logarithmic scale). The bottleneck segment length $\ell_s$ (labels)
contracts from $43$ at $D=1$ to $11$ at $D=4$, and the transfer accelerates by up
to $\times13134$. (b) Collective CTAP for the $D=4$ chain ($L=45$, five protected
states): a single counter-intuitive pulse sequence drives $P_L$ (blue) into
$P_R$ (red) with fidelity $F=0.9976$, while the inner solitons remain dark
($P_{S_1},P_{S_3}\approx0$; $P_{S_2}$, orange, only transiently reaches
$\approx0.24$ at $t/T=\tfrac12$). The transfer uses nearest-neighbor couplings
only, preserving chiral symmetry.}
\label{fig:multidomain}
\end{figure*}

\subsection{Chiral symmetry and disorder robustness}
\label{sec:robustness}

The defining feature of the SSH edge states is that chiral (sublattice) symmetry
pins them at $E=0$. Because CTAP transports population through these zero modes
using nearest-neighbor couplings only, it should inherit that protection; the
counterdiabatic protocol of Sec.~\ref{sec:cd_protocol}, by contrast, relies on
next-nearest-neighbor (NNN) hoppings that break chiral symmetry. We make the
contrast quantitative in two steps.

We first ask how the zero mode itself responds to perturbations, by diagonalizing
the perturbed Hamiltonian and tracking the mean magnitude of its energy
$\langle|E_0|\rangle$ [Fig.~\ref{fig:robustness}(a)]. Off-diagonal (hopping)
disorder---which preserves chiral symmetry---leaves the mode pinned at $E=0$ to
machine precision ($\sim\!10^{-16}$) for all disorder strengths. On-site disorder
and NNN hoppings, which both break chiral symmetry, instead detach the mode
immediately; the NNN term does so linearly, $\langle|E_0|\rangle\approx
0.95\,\kappa$.\footnote{The next-nearest-neighbor (NNN) perturbation used in Fig.~\ref{fig:robustness} is a \emph{real} hopping between same-sublattice sites, taken as a representative member of the chiral-symmetry-breaking class. The counterdiabatic term of Eq.~(\ref{eq:cd_nnn}) is the imaginary, sublattice-$A$ instance of the same class; the linear detachment of the zero mode is qualitatively identical in both cases, while the slope $\langle|E_0|\rangle\approx0.95\,\kappa$ quoted here is specific to the real perturbation.} The pinning is thus a sharp, symmetry-selective effect: only
perturbations that respect chiral symmetry preserve the zero mode.

This spectral pinning carries directly over to the transfer
[Fig.~\ref{fig:robustness}(b)]. Working at a deep adiabatic margin (so that the
disorder-free fidelity exceeds $0.99$ in every case) and averaging over disorder
realizations (we denote the disorder-averaged transfer fidelity
$\langle f\rangle$), we find that off-diagonal disorder barely affects CTAP:
$\langle f\rangle=0.994,\,0.994,\,0.993$ at $\sigma=0.05,\,0.10,\,0.20$.
On-site disorder degrades it steadily---$0.975,\,0.774,\,0.302$ over the same
range---with a large realization-to-realization spread, the signature of the zero
mode wandering away from $E=0$. Adding a moderate NNN coupling $\kappa=J/3$ does
\emph{not} erode the off-diagonal robustness: the fidelity stays flat at
$\approx0.99$, because such an NNN term is perturbatively small against the bulk
gap ($\sim0.5$) and shifts the protocol only negligibly.

Together these results sharpen the speed--robustness trade-off between the two
paradigms. The protection that survives off-diagonal disorder is exactly the
chiral pinning of the zero mode, and CTAP---being nearest-neighbor---retains it
in full. The counterdiabatic protocol buys its chain-length-independent speed
through NNN driving at strengths $\kappa\sim\lVert H_0\rVert$
(Sec.~\ref{sec:cd_protocol}), comparable to the hoppings themselves; the spectral
panel shows that in that regime the zero mode is strongly detached
($\langle|E_0|\rangle\sim0.95\,\kappa$). The acceleration of the CD protocol is
therefore obtained at the expense of the chiral pinning that underlies
off-diagonal-disorder robustness---a trade-off that CTAP, operating entirely
within the nearest-neighbor topological subspace, avoids.

A distinct robustness axis is the placement of the domain wall itself. A
$\pm2$-site misplacement, which changes the segment lengths and hence the
effective couplings, reduces the static-Rabi fidelity to $0.052$ but the CTAP
fidelity only to $0.419$ at fixed, nominally calibrated pulses---roughly an order
of magnitude more tolerant. This reflects the asymmetry invariance of
Sec.~\ref{sec:ctap_advantages}: the coupling ratio sets only the adiabatic margin of
CTAP, not its achievable fidelity, so a positional error that is fatal to the
static Rabi protocol merely detunes CTAP. This ``geometric'' robustness is
independent of the symmetry-protection mechanism above.

\begin{figure*}[!t]
\centering
\includegraphics[width=\textwidth]{figures/fig_robustness_chiral.pdf}
\caption{Chiral symmetry and disorder robustness. (a) Mean zero-mode energy
$\langle|E_0|\rangle$ (log scale) under off-diagonal (hopping) disorder $\sigma$
at $\kappa=0$ (blue, bottom axis), on-site disorder $\sigma$ at $\kappa=0$
(orange, bottom axis), and an NNN coupling $\kappa$ at $\sigma=0$ (teal, top
axis). Off-diagonal disorder respects chiral symmetry and pins the mode at $E=0$
to machine precision ($\sim\!10^{-16}$); on-site disorder and NNN coupling break
it and detach the mode, the latter linearly as $\langle|E_0|\rangle\approx
0.95\,\kappa$. (b) Mean CTAP transfer fidelity $\langle f\rangle$ versus
disorder strength $\sigma$ (error bars: standard deviation over realizations).
Off-diagonal disorder leaves the fidelity flat at $\approx0.99$ (blue), and it
stays flat when a moderate NNN coupling $\kappa=J/3$ is added (teal); on-site
disorder degrades it to $\approx0.30$ at $\sigma=0.20$ (orange). Dotted line:
$f_0=0.995$.}
...
\label{sec:ctap_implementation}

\textbf{Effective couplings by projection.}
The instantaneous couplings $\JLS(t)$ and $\JSR(t)$
[Eqs.~(\ref{eq:ctap_jsr})--(\ref{eq:ctap_jls})] are extracted from the full
Hamiltonian at every time step. We diagonalize $H(t)$, sort the eigenstates by
$|E|$ and retain the three of smallest energy---selected by magnitude rather
than by a fixed energy cutoff, so that the procedure stays valid even as the
protected gap narrows. These three states are labeled $\ketL,\ketS,\ketR$
according to the position of their probability-density maximum and rotated to
the maximally localized (Wannier) basis by diagonalizing the position operator
restricted to the three-dimensional protected subspace. The couplings $\JLS$
and $\JSR$ are then read off as the off-diagonal elements of $H(t)$ projected
onto this localized basis.

\textbf{Bulk leakage.}
At every time step we project $|\psi(t)\rangle$ onto the instantaneous protected
triplet and evaluate
$1-(P_{\mathcal{L}}+P_{\mathcal{S}}+P_{\mathcal{R}})$. The leakage quoted in the
figures and in Table~\ref{tab:fullchain} is the \emph{maximum} of this quantity
over the full trajectory.

\textbf{Disorder averaging.}
The spectral-pinning data [Fig.~\ref{fig:robustness}(a)] are averaged over
$N_{\mathrm{real}}=200$ disorder realizations; the disorder-averaged transfer
fidelity [Fig.~\ref{fig:robustness}(b)] uses $N_{\mathrm{real}}=40$
realizations.

\textbf{CTAP pulse parameters.}
The Gaussian pulse parameters [Eqs.~(\ref{eq:ctap_jsr})--(\ref{eq:ctap_jls})]
vary between figures:
\begin{itemize}
  \item \emph{Fig.~\ref{fig:ctap_pulses_dynamics} (effective three-level demo,
    $T=300$)}: $\sigma=0.18\,T$, $t_R=T/3$, $t_L=2T/3$
    ($t_L-t_R=T/3\approx1.85\,\sigma$).
  \item \emph{Fig.~\ref{fig:ctap_comparison} ($f$ vs.\ $j_{\rm DW}$,
    effective three-level)}: well-conditioned (WC) pulse, $\sigma=0.12\,T$,
    $t_R=T/4$, $t_L=3T/4$ (separation $T/2$); bare CTAP with $T=800\rho$
    (scaling with the coupling ratio at each position) and CD-assisted CTAP at
    fixed $T=600$.
  \item \emph{Fig.~\ref{fig:ctap_asymmetry} ($f$ vs.\ $\rho$)}: two variants.
    The original pulse ($\sigma=0.18\,T$, $t_R=T/3$, $t_L=2T/3$, $T=400\rho$) is
    shown as the comparison curve tracking $\cos^2\Theta_0(\rho)$; the WC pulse
    ($\sigma=0.12\,T$, $t_R=T/4$, $t_L=3T/4$, fixed $T=5\times10^{4}$) confirms
    $f\approx1$ across the whole range of $\rho$.
  \item \emph{Table~\ref{tab:fullchain} / Fig.~\ref{fig:fullchain} (full chain,
    $L=21$)}: for $j_{\rm DW}=11,7$, $\sigma=0.15\,T$, $t_R=0.40\,T$,
    $t_L=0.60\,T$ ($t_L-t_R=0.20\,T$); for $j_{\rm DW}=5$, $\sigma=0.18\,T$,
    $t_R=0.32\,T$, $t_L=0.68\,T$ ($t_L-t_R=0.36\,T$). The protocol duration is
    fixed by $T=\tau_{\mathrm{ad}}/J_{\mathrm{bott}}$ with
    $\tau_{\mathrm{ad}}\in\{180,300,450\}$, the smallest adiabatic margin for
    which $f>0.995$ and $P_{\mathcal{S}}^{\max}<0.02$.
\end{itemize}

\textbf{Time integration.}
The full-chain dynamics are integrated with an adaptive Runge--Kutta scheme
(RK45, $\mathrm{rtol}=10^{-6}$, $\mathrm{atol}=10^{-8}$); the effective
three-level dynamics, which demand higher accuracy, use an eighth-order
Runge--Kutta integrator (DOP853, $\mathrm{rtol}=10^{-11}$,
$\mathrm{atol}=10^{-13}$, maximum step $\Delta t_{\max}=T/4000$). The complete
set of scripts that reproduce every figure and table, together with the
generated data, is openly available in the public repository accompanying this
work.\footnote{\url{https://github.com/miguelgarciamier/Quantum-control-for-quantum-information-transfer-mediated-by-topological-edge-states}}

% ============================================================================
\section{Discussion and conclusions}
\label{sec:conclusions}

We have presented a detailed study of quantum state transfer in SSH chains,
comparing two distinct paradigms: domain wall-mediated transfer following
J.~Zurita \textit{et al.}~\cite{Zurita2023} and hybrid analog-digital
...
For the symmetric two-domain configuration ($L = 11$, $N = 2$, $\ell = 4$), we
faithfully reproduce the results of Ref.~\cite{Zurita2023}: the optimal transfer
time $\ttr = 45$ agrees with the analytical prediction $\ttr \approx 45.6$ and
the fidelity $f = 0.999$ exceeds the high-fidelity benchmark $f_0 = 0.995$,
providing a validated numerical platform for the subsequent analysis.
Our five main conclusions are:

\textbf{1.~Static Rabi transfer fails under coupling asymmetry.}
Displacing the domain wall from the chain center generates an exponentially
large coupling ratio $J_1/J_2 \sim (w/v)^{|\ell_1 - \ell_2|/2}$, which bounds
the static Rabi fidelity by $f_{\max} = 4J_1^2 J_2^2/(J_1^2 + J_2^2)^2$.
A single unit-cell displacement in $L = 21$ reduces the fidelity from
$\sim\!1$ to $0.637$; larger displacements make the transfer negligible.
This is \emph{not a topological constraint}: the three edge states
($\ketL$, $\ketS$, $\ketR$) remain exponentially localized and well-separated
from the bulk at all configurations studied. The limitation is an artifact of
the static Rabi protocol, which couples both domains through a single hopping
parameter $v(t)$ and thus cannot independently tune $\JLS$ and $\JSR$.

\textbf{2.~Ramp shaping cannot overcome the coupling-asymmetry bound.}
STA-inspired ramp shaping (Global $\sin^2$ ramp, modified-exponent
Ramp-shaping ($\alpha$), and linear ramps) provides marginal improvements
in the symmetric case---the Global $\sin^2$ ramp achieves $f = 0.99995$---but
is completely ineffective against coupling asymmetry: no ramp variant reduces
the infidelity by more than statistical noise at any asymmetric wall position.
Ramp shaping acts only on the preparation and readout phases; the transfer
phase is governed by the fixed effective couplings $\JLS$ and $\JSR$ set by
the domain geometry [Eq.~(\ref{eq:coupling_ratio})], which no envelope choice
can alter.

\textbf{3.~CTAP exploits the topological dark state to bypass the asymmetry.}
Introducing independent controls $v_L(t)$ and $v_R(t)$ for the two domains
replaces the static Rabi oscillation with adiabatic evolution along the
zero-energy dark state $\ket{D_0(t)} = \cos\Theta(t)\,\ketL -
\sin\Theta(t)\,\ketR$---a consequence of chiral symmetry that holds
regardless of the coupling ratio.
Full-chain integration of the $L=21$ SSH chain with the counter-intuitive
Gaussian pulse sequence recovers $f > 0.999$ at every wall position where the
Rabi fidelity collapses to $\lesssim10^{-2}$, with bulk leakage below
$10^{-7}$ and an effective three-level reduction accurate to better than
$0.025\%$; the residual infidelity is a removable pulse boundary-condition
effect ($f \leq \cos^2\Theta_0$), not a fundamental limit, and the only
genuine bound ($\rho \gtrsim 10^2$) arises from geometric gap closing.
The effective counterdiabatic correction
$\mathcal{H}_{\mathrm{CD}}^{\mathrm{eff}} = i\hbar\dot{\Theta}
(\ketL\bra{\mathcal{R}} - \ketR\bra{\mathcal{L}})$, acting on the
three-dimensional protected subspace, enables perfect transfer in arbitrarily
short times and is far more tractable than full-chain CD optimization.

\textbf{4.~Multi-domain subdivision gives exponential acceleration with a
collective dark state.}
Subdividing the chain into $D$ domains at fixed total length shrinks the
bottleneck segment and reduces the transfer time by more than four orders
of magnitude ($\sim\!1.3\times10^4$ at $D = 4$).
A single counter-intuitive collective pulse sequence achieves $F = 0.998$
across the four-domain chain, keeping the inner soliton states dark and
using nearest-neighbor couplings only---so the speed-up is obtained without
sacrificing the chiral symmetry that underlies topological protection.

\textbf{5.~Chiral symmetry, not topology alone, governs disorder robustness.}
Chiral symmetry pins the zero-energy edge states exactly at $E = 0$:
off-diagonal (hopping) disorder leaves the zero mode at machine precision and
the CTAP fidelity flat at $\langle f\rangle \approx 0.99$, whereas
on-site disorder and next-nearest-neighbor hoppings break the symmetry and
linearly detach the mode ($\langle|E_0|\rangle \approx 0.95\,\kappa$ for NNN
coupling $\kappa$), degrading the transfer.
Because CTAP uses only nearest-neighbor couplings it fully inherits this
chiral protection, while the NNN-based CD protocol of Ref.~\cite{Romero2024}
sacrifices it to gain speed: the control protocol, not the topological phase
alone, determines whether transfer is robust.
CTAP is also $\sim\!8\times$ more tolerant to domain-wall misplacement than
the static Rabi protocol.

The CD protocol of Ref.~\cite{Romero2024}---achieving fidelities above $99\%$
for chains up to nine sites and $\sim\!90\%$ for fifteen sites at a
$100\times$ speed-up over the adiabatic protocol---constitutes a complementary
paradigm suited to uniform chains with tuneable NNN interactions and stringent
time constraints, at the cost of the chiral protection highlighted in
conclusion~5.
For chains with two domain walls, the even-state parity constraint precludes
a straightforward single-dark-state CTAP protocol; the Straddling-CTAP scheme
of Ref.~\cite{Greentree2004}, combining a persistent inter-soliton coupling
with counter-intuitive outer pulses, offers a route forward whose optimization
and experimental verification we leave for future work.
Together, these results demonstrate that topological quantum state transfer
can be made simultaneously fast, asymmetry-resilient, and disorder-robust by
choosing control protocols that respect the chiral symmetry of the underlying
lattice---a design principle applicable across superconducting qubit,
