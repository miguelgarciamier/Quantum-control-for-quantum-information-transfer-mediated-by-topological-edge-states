...
paradigm proposed by S.~V.~Romero, X.~Chen, G.~Platero, and
Y.~Ban~\cite{Romero2024}. Rather than exploiting topological domain walls as
...
S.~V.~Romero \textit{et al.}~\cite{Romero2024} employ the variational approach
to approximate CD driving developed by D.~Sels and A.~Polkovnikov~\cite{Sels2017}. The approximate CD Hamiltonian at order $l$
takes the form
\begin{equation}
...
To circumvent the limitations of the analytical NC approach, S.~V.~Romero
...
approach of Sec.~\ref{sec:cd_driving}, applied now to the effective rather
...
\begin{figure}[t]
  ...
  \caption{Transfer fidelity versus domain wall position $j_{\mathrm{DW}}$
  for $L = 21$ ($v = 0.5$, $w = 1.0$).  The static Rabi protocol (red
  circles) peaks near $f \approx 0.64$ close to the chain quasi-centre and
  collapses exponentially to $f \approx 0.004$ at $j_{\mathrm{DW}} = 5$, the
  value computed in Sec.~\ref{sec:asym_fidelity}.  The CTAP protocol (blue
  diamonds) maintains $f \approx 0.99$ at \emph{all} valid domain wall
  positions, demonstrating complete immunity to the coupling asymmetry.  The
  shaded region represents the fidelity gain obtained by switching from the
  static to the dynamic protocol.}
...
$\Theta(t)$ from $0$ to $\pi/2$, achieving near-perfect fidelity regardless
of the coupling ratio. The fidelity is bounded only by the adiabaticity
condition, which can always be satisfied by sufficiently slow pulses---even
for highly asymmetric domain wall positions.
