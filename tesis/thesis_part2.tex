...
nearest-neighbour couplings it inherits the chiral protection that pins the edge
states against off-diagonal disorder, whereas the next-nearest-neighbour driving
...
it consists of next-to-nearest-neighbor (NNN) hoppings between sites of the \emph{same} sublattice. Related schemes that accelerate and stabilize SSH edge-state transfer with engineered NNN couplings have also been explored~\cite{DAngelis2020}.
...
predicts. The protocol uses nearest-neighbour couplings only, so it inherits the
...
$\approx0.24$ at $t/T=\tfrac12$). The transfer uses nearest-neighbour couplings
...
using nearest-neighbour couplings only, it should inherit that protection; the
counterdiabatic protocol of Sec.~\ref{sec:cd_protocol}, by contrast, relies on
next-nearest-neighbour (NNN) hoppings that break chiral symmetry. We make the
...
realizations, we find that off-diagonal disorder barely affects CTAP:
$\langle\mathcal{F}\rangle=0.994,\,0.994,\,0.993$ at $\sigma=0.05,\,0.10,\,0.20$.
...
chiral pinning of the zero mode, and CTAP---being nearest-neighbour---retains it
...
within the nearest-neighbour topological subspace, avoids.
...
0.95\,\kappa$. (b) Mean CTAP transfer fidelity $\langle\mathcal{F}\rangle$ versus
...
using nearest-neighbour couplings only---so the speed-up is obtained without
...
the CTAP fidelity flat at $\langle\mathcal{F}\rangle \approx 0.99$, whereas
on-site disorder and next-nearest-neighbour hoppings break the symmetry and
linearly detach the mode ($\langle|E_0|\rangle \approx 0.95\,\kappa$ for NNN
coupling $\kappa$), degrading the transfer.
Because CTAP uses only nearest-neighbour couplings it fully inherits this
...
with counter-intuitive outer pulses, offers a route forward whose optimisation
