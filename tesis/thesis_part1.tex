...
\begin{figure*}[t]
  ...
  \textbf{(c)} $j_{\mathrm{DW}} = 5$, each at its optimal $\ttr$ (Table~\ref{tab:asym_fidelity}).
  Each panel shows the shaded ramp $v(t)$ (left axis) and the occupations of the
  three localized sites---site $0$ ($\ketL$), the domain wall ($\ketS$) and site
  $L-1$ ($\ketR$) (right axis). As the wall moves off-centre, the coupling
  asymmetry $\JLS \neq \JSR$ caps the population that can be swapped into $\ketR$:
  the transfer is essentially complete in (a) but increasingly incomplete in (b)
  and (c), where the excitation remains trapped in $\ketL$ and $\ketS$.}
...
geometry [Eq.~(12)] and not by the ramp profile.} Reshaping the ramp therefore
cannot alter the bound $f_{\max}$ of Eq.~(17): only a protocol that varies the
coupling ratio $\JLS(t)/\JSR(t)$ in time---such as the CTAP scheme of
Sec.~\ref{sec:ctap}---can overcome the asymmetry.
