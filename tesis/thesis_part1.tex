...
First, we numerically reproduce the domain wall protocol of
Zurita \textit{et al.}~[Quantum \textbf{7}, 1043 (2023)],
...
demonstrating that the fidelity is fundamentally limited by the exponential
sensitivity of the effective couplings to the domain lengths.
...
transfer phase.
Second, we present a comprehensive analysis of the hybrid analog-digital protocol
of Romero \textit{et al.}~[Phys.\ Rev.\ Applied \textbf{21}, 034033 (2024)],
where nonadiabatic edge-state transfer is achieved by supplementing the SSH
Hamiltonian with approximate counterdiabatic interactions derived from adiabatic
gauge potentials and optimized via variational quantum circuits.
We offer a systematic comparison of both approaches, contrasting their physical
mechanisms, scalability, experimental requirements, and robustness against
disorder. The domain wall approach exploits topological protection within an
effective low-energy subspace, while the CD approach directly engineers the
time evolution to follow the instantaneous eigenstate, achieving fidelities
above 99\% for chains up to 15 sites in the nonadiabatic regime.
\end{abstract}
...
Zurita, Creffield, and Platero~\cite{Zurita2023} demonstrated that chains
...
the boundary states, represents an efficient and robust approach for quantum
information transport.

In this work, we investigate topological quantum state transfer in SSH chains.
We focus on two-domain ($N = 2$) configurations and address three
questions:
\begin{enumerate}
...
Romero~\textit{et al.}~\cite{Romero2024}, where nonadiabatic transfer is
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{fig1_dimer_spectrum.pdf}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{fig2_domain_wall_states.pdf}
  \caption{Topologically protected states in a two-domain SSH chain ($L = 11$,
  $N = 2$, $\ell = 4$, $v = 0.5$, $w = 1.0$). The three states $\ketL$,
...
The transfer protocol proposed by Zurita \textit{et al.}~\cite{Zurita2023}
...
The initial state $\ket{0}$ evolves through the three
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{fig5_asymmetric_wall_states.pdf}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{fig8_effective_coupling.pdf}
...
The coupling asymmetry has a direct and significant consequence for the transfer
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{fig6_asymmetric_transfer.pdf}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{fig7_fidelity_comparison.pdf}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{fig12_systematic_wall_position.pdf}
  \caption{Maximum transfer fidelity as a function of domain wall position for
  $L = 11$ (left) and $L = 15$ (right). The fidelity is sharply peaked at the
  chain center and symmetric about it. Only odd-index sites produce valid domain
  walls.}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{fig9_sta_pulse_comparison.pdf}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{fig10_tprep_dependence.pdf}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{fig11_combined_study.pdf}
...
preparation and readout fidelity; the transfer dynamics are governed entirely
by the topological properties of the chain.

% ============================================================================
\section{Hybrid analog-digital counterdiabatic protocol}
\label{sec:cd_protocol}

In this section, we present a detailed analysis of the alternative transfer
paradigm proposed by Romero, Chen, Platero, and
Ban~\cite{Romero2024}. Rather than exploiting topological domain walls as
in the Zurita protocol (Sec.~\ref{sec:transfer}), this approach achieves
nonadiabatic edge-state transfer in a \emph{uniform} SSH chain by supplementing
the Hamiltonian with approximate counterdiabatic (CD) interactions. The CD
terms are derived analytically from adiabatic gauge potentials and can be
further optimized digitally via variational quantum circuits, hence the
denomination ``hybrid analog-digital.''

% --- 6.1 SSH model with odd sites ---
\subsection{SSH chain with an odd number of sites}
\label{sec:cd_ssh}

The starting point is a standard SSH chain with $2N - 1$ sites (an odd number),
with $N$ sites on sublattice A (odd indices) and $N - 1$ on sublattice B
(even indices). The Hamiltonian is~\cite{Romero2024}
\begin{equation}
  H_0(t) = t_2(t) \sum_{j=1}^{N-1} c_{2j}^\dagger c_{2j-1}
  + t_1(t) \sum_{j=1}^{N-1} c_{2j+1}^\dagger c_{2j} + \text{H.c.},
  \label{eq:H0_romero}
\end{equation}
where $c_i^\dagger$ ($c_i$) creates (annihilates) a fermion at site $i$,
$t_2(t)$ is the intra-cell hopping, and $t_1(t)$ is the inter-cell hopping.
Due to chiral symmetry, the spectrum consists of $2N - 2$ nonzero-energy
levels arranged symmetrically about $E = 0$, plus one exact zero-energy
eigenstate~\cite{Asboth2016}:
\begin{equation}
  \ket{\Phi_0(t)} = \frac{1}{\mathcal{N}} \sum_{i=1}^{N}
  \left[-\frac{t_2(t)}{t_1(t)}\right]^{i-1} \ket{A_i},
  \label{eq:zero_mode_romero}
\end{equation}
where $\mathcal{N}^2 = \sum_{i=0}^{N-1} [t_2(t)/t_1(t)]^{2i}$ is the
normalization factor. Crucially, this zero-energy state has nonzero components
only on sublattice A sites (the $b_i = 0$ property), a consequence of chiral
symmetry that will be exploited in the simplification of the CD terms.

The transfer mechanism is conceptually simple: when $t_1(0)/t_2(0) \gg 1$, the
zero-energy state is localized at the left edge (site $A_1$). When
$t_1(T)/t_2(T) \ll 1$ at the final time $t = T$, the state migrates to the
right edge (site $A_N$). An adiabatic modulation of the hopping parameters
achieves this transfer, as in the original Thouless
pumping~\cite{Thouless1983, Longhi2019}. A standard parametrization
is~\cite{Romero2024}
\begin{equation}
  t_{1,2}(t) = t_0(1 \pm \cos\Omega t),
  \label{eq:hoppings_romero}
\end{equation}
where $\Omega$ sets the speed of the protocol and the adiabatic regime
corresponds to $\Omega = 0.01\,t_0$ (transfer time $T = \pi/\Omega =
100\pi/t_0$). The transfer fidelity is
\begin{equation}
  F(t) = |\langle \Phi_0(t) | \Psi(t) \rangle|^2,
  \label{eq:fidelity_romero}
\end{equation}
measuring how closely the evolved state $\ket{\Psi(t)}$ follows the
instantaneous zero-energy eigenstate at all times during the protocol.

It is important to note two key differences with the Zurita
protocol (Sec.~\ref{sec:protocol}): (i) the chain contains no domain
wall---it is a uniform SSH chain with a single dimerization pattern; and
(ii) the transfer operates by dynamically sweeping the hopping ratio
$t_1(t)/t_2(t)$ through the entire topological phase diagram, rather than
by populating the protected subspace at a fixed hopping ratio. The adiabatic
condition $T \gg \pi/t_0$ ensures that the state tracks the instantaneous
eigenstate, but this requires very long protocol times, making the system
vulnerable to decoherence.

% --- 6.2 Counterdiabatic driving ---
\subsection{Counterdiabatic driving from adiabatic gauge potentials}
\label{sec:cd_driving}

The central idea of counterdiabatic (CD) driving~\cite{Demirplak2003,
Berry2009} is to add an auxiliary Hamiltonian $H_{\mathrm{cd}}(t)$ to the
original $H_0(t)$ such that the total evolution under
$H(t) = H_0(t) + H_{\mathrm{cd}}(t)$ exactly tracks the instantaneous
eigenstate of $H_0(t)$, even when the adiabatic condition is violated. The
exact CD term~\cite{Berry2009} requires full spectral knowledge of $H_0(t)$
at all times and typically involves highly nonlocal interactions, making it
impractical.

Romero \textit{et al.}~\cite{Romero2024} employ the variational approach
to approximate CD driving developed by Sels and
Polkovnikov~\cite{Sels2017}. The approximate CD Hamiltonian at order $l$
takes the form
\begin{equation}
  H_{\mathrm{cd}}^{(l)} = \dot{\lambda}\, \mathcal{A}_\lambda^{(l)},
  \label{eq:Hcd}
\end{equation}
where $\dot{\lambda} = \dot{t}_1(t)$ is the rate of change of the control
parameter and $\mathcal{A}_\lambda^{(l)}$ is the approximate adiabatic gauge
potential, expanded in terms of nested commutators (NCs) of $H_0$ and
$\partial_\lambda H_0$:
\begin{equation}
  \mathcal{A}_\lambda^{(l)} = i \sum_{k=1}^{l} \alpha_k^{(l)}
  \underbrace{[H_0, [H_0, \ldots, [H_0}_{2k-1},
  \partial_\lambda H_0]]].
  \label{eq:gauge_potential}
\end{equation}
The time-dependent coefficients $\alpha_k^{(l)}(t)$ are determined variationally
by minimizing the action $S_l = \mathrm{tr}[G_l^2]$, where
$G_l = \partial_\lambda H_0 - i[H_0, \mathcal{A}_\lambda^{(l)}]$. In the limit
$l \to \infty$, the expansion converges to the exact gauge potential. For finite
$l$, one obtains an approximate CD driving whose accuracy improves with order.

% --- 6.3 The first-order NC ---
\subsection{First-order nested commutator}
\label{sec:first_order}

The first-order ($l = 1$) gauge potential involves only the single commutator
$[H_0, \partial_\lambda H_0]$, yielding~\cite{Romero2024}
\begin{equation}
  \mathcal{A}_\lambda^{(1)} = i\kappa^{(1)} \left[
  \sum_{j=1}^{N-2} c_{2j+2}^\dagger c_{2j}
  - \sum_{j=1}^{N-1} c_{2j+1}^\dagger c_{2j-1} \right] + \text{H.c.},
  \label{eq:A1}
\end{equation}
where $\kappa^{(1)} = -\Lambda\dot{\lambda}\,\alpha_1^{(1)}$ with
$\Lambda = 2t_0$. The variational parameter is obtained
analytically~\cite{Romero2024, Sels2017} as
\begin{equation}
  \alpha_1^{(1)} = -\frac{\mathcal{C}(N)}{t_1^2 + t_2^2},
  \quad
  \mathcal{C}(N) = \frac{2(N-2) + 1}{8(N-2) + 1}.
  \label{eq:alpha1}
\end{equation}

The physical interpretation of Eq.~(\ref{eq:A1}) is highly significant:
\emph{the first-order CD driving consists of next-to-nearest-neighbor (NNN)
hopping terms between sites of the same sublattice}---odd-odd (sublattice A)
and even-even (sublattice B) with equal magnitude $|\kappa^{(1)}|$.
Since the zero-energy eigenstate [Eq.~(\ref{eq:zero_mode_romero})] has zero
amplitude on all sublattice B sites, the NNN hoppings on sublattice B do not
contribute to the transfer and can be neglected. This simplification reduces
the CD driving to NNN hoppings exclusively between neighboring sublattice A
sites (Fig.~\ref{fig:cd_schematic}), which is far more experimentally
tractable than the generic nonlocal interactions of the exact CD
Hamiltonian~(\ref{eq:cd}).

\begin{figure}[t]
  \centering
  \begin{tikzpicture}[
    siteA/.style={circle, draw=blue!70, fill=blue!15, minimum size=6mm, inner sep=0pt, font=\footnotesize},
    siteB/.style={circle, draw=red!70, fill=red!15, minimum size=6mm, inner sep=0pt, font=\footnotesize},
    hop/.style={thick},
    nnn/.style={thick, dashed, blue!60},
    >=stealth
  ]
    % Sites
    \node[siteA] (A1) at (0,0) {$A_1$};
    \node[siteB] (B1) at (1.2,0) {$B_1$};
    \node[siteA] (A2) at (2.4,0) {$A_2$};
    \node[siteB] (B2) at (3.6,0) {$B_2$};
    \node[siteA] (A3) at (4.8,0) {$A_3$};
    \node[siteB] (B3) at (6.0,0) {$B_3$};
    \node[siteA] (A4) at (7.2,0) {$A_4$};
    % NN hoppings
    \draw[hop] (A1) -- node[above, font=\scriptsize] {$t_2$} (B1);
    \draw[hop] (B1) -- node[above, font=\scriptsize] {$t_1$} (A2);
    \draw[hop] (A2) -- node[above, font=\scriptsize] {$t_2$} (B2);
    \draw[hop] (B2) -- node[above, font=\scriptsize] {$t_1$} (A3);
    \draw[hop] (A3) -- node[above, font=\scriptsize] {$t_2$} (B3);
    \draw[hop] (B3) -- node[above, font=\scriptsize] {$t_1$} (A4);
    % NNN hoppings (CD driving) on sublattice A
    \draw[nnn] (A1) to[bend left=45] node[above, font=\scriptsize] {$\kappa_{31}$} (A2);
    \draw[nnn] (A2) to[bend left=45] node[above, font=\scriptsize] {$\kappa_{53}$} (A3);
    \draw[nnn] (A3) to[bend left=45] node[above, font=\scriptsize] {$\kappa_{75}$} (A4);
    % Labels
    \node[below=3mm, font=\scriptsize, blue!70] at (1.2, -0.55) {Sublattice A: odd sites};
    \node[below=3mm, font=\scriptsize, red!70] at (5.4, -0.55) {Sublattice B: even sites};
    % Braces for unit cell
    \draw[decorate, decoration={brace, amplitude=4pt, mirror}]
      ($(A1.south) + (-0.1, -0.15)$) -- ($(B1.south) + (0.1, -0.15)$)
      node[midway, below=5pt, font=\scriptsize] {unit cell};
  \end{tikzpicture}
  \caption{Schematic of the hybrid analog-digital protocol for a chain of
  $2N - 1 = 7$ sites. The original SSH hopping parameters $t_1(t)$, $t_2(t)$
  (solid lines) are supplemented by counterdiabatic NNN hoppings
  $\kappa_{2j+1,2j-1}$ (dashed arcs) connecting neighboring sublattice A
  sites. The NNN terms between sublattice B sites are negligible
  for the zero-energy eigenstate transfer.}
  \label{fig:cd_schematic}
\end{figure}

% --- 6.4 Higher-order contributions ---
\subsection{Higher-order nested commutators}
\label{sec:higher_order}

The first-order NC provides excellent results for short chains (five sites)
but becomes insufficient for longer chains, where higher-order corrections
are needed. The second-order NC ($l = 2$) introduces additional terms:
\begin{equation}
  \mathcal{A}_\lambda^{(2)} = i\bigl(
  \alpha_1^{(2)} [H_0, \partial_\lambda H_0]
  + \alpha_2^{(2)} [H_0, [H_0, [H_0, \partial_\lambda H_0]]]
  \bigr),
  \label{eq:A2}
\end{equation}
which, after evaluation, produces NNN hoppings (as from the first order)
plus fourth-nearest-neighbor hoppings within and between sublattices. Again,
the sublattice B terms can be neglected for the zero-energy state transfer.

In general, the $l$th-order NC introduces interactions up to $2l$th-nearest
neighbors~\cite{Romero2024}. For a chain with $2N - 1$ sites, the minimum
order required to account for all possible interactions along the same
sublattice is $l = N - 1$. This rapid growth of computational complexity and
long-range interactions motivates two key simplifications performed
in Ref.~\cite{Romero2024}:

\begin{enumerate}
  \item \textbf{Neglect sublattice B hoppings}: Since the transfer trajectory
    follows the zero-energy eigenstate, which has zero amplitude on sublattice B,
    the CD hoppings between even-even sites do not contribute.
  \item \textbf{Retain only NNN hoppings on sublattice A}: The longer-range
    hoppings (fourth-nearest neighbors and beyond) from higher-order NCs are
    discarded, keeping only the dominant NNN terms. The simplified CD driving
    takes the form
    \begin{equation}
      H_{\mathrm{cd}} = -i \sum_{j=1}^{N-1}
      \kappa_{2j+1,2j-1}\, c_{2j+1}^\dagger c_{2j-1} + \text{H.c.},
      \label{eq:Hcd_simplified}
    \end{equation}
    where $\kappa_{2j+1,2j-1}$ are site-dependent coupling strengths to be
    optimized.
\end{enumerate}

This simplified form retains the essential physics of the CD correction while
requiring only experimentally accessible NNN hoppings. For a five-site chain,
this simplified protocol achieves fidelities indistinguishable from the full
second-order NC, confirming that the NNN sublattice A terms capture the
dominant CD contribution.

% --- 6.5 Numerical results: analog CD ---
\subsection{Analog counterdiabatic results}
\label{sec:cd_analog_results}

For a five-site chain ($N = 3$) in the nonadiabatic regime ($\Omega = t_0$,
$T = \pi$), the performance of the different CD approximations can be summarized as follows~\cite{Romero2024}:

\begin{itemize}
  \item \textbf{No CD} ($H_0$ only): The fidelity drops
    significantly in the nonadiabatic regime, falling below 50\% for
    $\Omega \gtrsim 0.1\,t_0$.
  \item \textbf{First-order NC} ($H_0 + H_{\mathrm{cd}}^{(1)}$): The transfer
    improves substantially but does not reach high fidelity for
    $\Omega = t_0$.
  \item \textbf{Second-order NC} ($H_0 + H_{\mathrm{cd}}^{(2)}$): The protocol
    retains high fidelity $F(T) > 99\%$ even at $\Omega = t_0$
    (transfer 100$\times$ faster than the adiabatic regime), with CD driving
    strengths of the same order as $t_0$.
\end{itemize}

These results demonstrate that a modest order of the NC expansion suffices
for short chains. However, for chains longer than five sites, higher orders
become necessary, and the analytical approach becomes computationally expensive.
This motivates the variational digital optimization described next.

% --- 6.6 Digital optimization ---
\subsection{Variational quantum circuit optimization}
\label{sec:cd_digital}

To circumvent the limitations of the analytical NC approach, Romero
\textit{et al.}~\cite{Romero2024} leverage digital quantum simulation (DQS)
to find optimal CD driving parameters. The key idea is to treat the
NNN hopping strengths $\kappa_{2j+1,2j-1}$ as variational parameters and
optimize them using classical optimizers in a hybrid
quantum-classical loop~\cite{Cerezo2021}.

\subsubsection{Encoding and Trotterization}

The SSH chain of $2N - 1$ sites is encoded in
$n = \lceil \log_2(2N-1) \rceil$ qubits. Both $H_0$ and $H_{\mathrm{cd}}$
are decomposed in the Pauli basis, and the time evolution is implemented
via first-order Trotter-Suzuki decomposition~\cite{Trotter1959}:
\begin{equation}
  U(T, 0) \approx \prod_{k=1}^{r}
  e^{-i H_0^{(c)}(k\Delta t)\Delta t}\,
  e^{-i H_{\mathrm{cd}}^{(c)}(k\Delta t)\Delta t},
  \label{eq:trotter}
\end{equation}
with $r = T/\Delta t$ Trotter steps. The NNN coupling strengths are
discretized as a vector of $r + 1$ parameters per hopping location, with
boundary conditions $\kappa(0) = \kappa(T) = 0$, leaving $r - 1$ free
parameters per location to optimize.

\subsubsection{Cost functions}

Two cost functions are employed~\cite{Romero2024}:

\begin{enumerate}
  \item \textbf{Transfer fidelity}: Directly maximizing
    $C(\vec{\kappa}^{(c)}) = F(T)$ [Eq.~(\ref{eq:fidelity_romero})], which
    requires computing state overlaps and is well-suited for noiseless
    simulators.
  \item \textbf{Hellinger distance}: A measurement-based cost function
    \begin{equation}
      H^2(P, Q) = 1 - \sum_{i=0}^{m-1} \sqrt{p_i q_i},
      \label{eq:hellinger}
    \end{equation}
    where $P$ is the measured probability distribution after $N_{\mathrm{shot}}$
    measurements and $Q$ is the target distribution (unit probability at the
    final site). Minimizing $H^2$ is equivalent to maximizing the population
    at the target site and is practical for real quantum
    hardware~\cite{Romero2024}.
\end{enumerate}

The optimization uses the simultaneous perturbation stochastic approximation
(SPSA) algorithm, initialized with values proportional to the analytical
first-order NC result $\kappa^{(1)}$ (``warm starting'') and with adaptive
bounds to keep $\|H_{\mathrm{cd}}\| \sim \|H_0\|$ for experimental
feasibility.

\subsubsection{Two simplification strategies}

Two simplification ansatzes are tested for the NNN hoppings:
\begin{enumerate}
  \item \textbf{Time-symmetric hoppings}: Exploiting the symmetry
    $\kappa_{2i+1,2i-1}^{(c)}(t) = \kappa_{2(N-i)+1,2(N-i)-1}^{(c)}(T - t)$,
    which halves the number of free parameters to
    $\lfloor N/2 \rfloor (r - 1)$.
  \item \textbf{Equal NNN hoppings}: All NNN couplings are set equal,
    $\kappa_{2j+1,2j-1}^{(c)}(t) \mapsto \kappa^{(c)}(t)$, reducing the
    optimization to only $r - 1$ parameters regardless of chain length.
    This is inspired by the site-independent structure of the first-order NC.
\end{enumerate}

\subsubsection{Results across chain lengths}

The variational circuit is implemented with $r = 22$ Trotter steps, with
10 independent runs per configuration to assess stability.
Table~\ref{tab:cd_fidelities} summarizes the optimal fidelities obtained
across chain lengths from 5 to 15 sites~\cite{Romero2024}.

\begin{table}[b]
  \caption{Transfer fidelities $F(T)$ from the variational quantum circuit
  optimization~\cite{Romero2024} for different chain lengths in the
  nonadiabatic regime ($\Omega = t_0$, $T = \pi$), using equal NNN hoppings
  on sublattice A with the transfer fidelity as cost function.}
  \label{tab:cd_fidelities}
  \begin{ruledtabular}
  \begin{tabular}{ccccc}
    Sites & $N$ & Qubits &
    \textrm{Iterations} & $\bar{F}(T)$ \\
    \colrule
    5  & 3 & 3 & 1000  & $> 0.999$ \\
    7  & 4 & 3 & 2000  & $> 0.99$  \\
    9  & 5 & 4 & 3000  & $> 0.99$  \\
    11 & 6 & 4 & 5000  & $\sim 0.97$ \\
    13 & 7 & 4 & 6000  & $\sim 0.94$ \\
    15 & 8 & 4 & 10000 & $\sim 0.90$ \\
  \end{tabular}
  \end{ruledtabular}
\end{table}

For short chains (5--9 sites), the optimized CD driving achieves near-perfect
fidelities with modest optimization effort. For longer chains (11--15 sites),
the fidelity decreases to approximately 90\% when the constraint
$\|H_{\mathrm{cd}}\| \sim \|H_0\|$ is maintained. Relaxing this constraint
(allowing stronger NNN hoppings) improves the fidelity at the expense of
experimental feasibility.

% --- 6.7 Transfer solely by CD driving ---
\subsection{Transfer solely by counterdiabatic driving}
\label{sec:cd_only}

A remarkable finding of Ref.~\cite{Romero2024} is that high-fidelity
transfer can be achieved by setting $H_0 = 0$ and driving the system
entirely through the CD terms. In this regime, the circuit contains
only the variational NNN hoppings on sublattice A, which simplifies the
implementation in several ways:
\begin{itemize}
  \item The circuit depth is significantly reduced (no $H_0$ block),
  \item Convergence is faster, requiring only 1000 iterations for all chain
    lengths tested,
  \item The fidelities consistently exceed 99\% for chains up to 15 sites,
    outperforming the $H_0 + H_{\mathrm{cd}}$ scheme.
\end{itemize}

This counterintuitive result can be understood as follows: when $H_0 = 0$,
the population is entirely confined to sublattice A (no excitation of
sublattice B sites), and the variational optimizer has full freedom to design
the transfer pathway without competition from the original Hamiltonian's
dynamics. The optimal CD driving in this case effectively encodes all the
information about the SSH topology into a purely sublattice A hopping
protocol.

% --- 6.8 Robustness ---
\subsection{Robustness against disorder}
\label{sec:cd_robustness}

The robustness of the CD protocol is assessed against three types of
perturbations~\cite{Romero2024}:

\begin{enumerate}
  \item \textbf{Rice-Mele staggered potential}: A diagonal term
    $\sum_j (-1)^{j-1}\Delta(t)\, c_j^\dagger c_j$ that breaks chiral
    symmetry. The fidelity remains above 90\% for $\delta/t_0 \lesssim 0.1$.
  \item \textbf{Diagonal disorder}: Random on-site energies
    $\delta_j \in [-\sigma_\delta, \sigma_\delta]$. High fidelity is
    preserved for $\sigma_\delta/t_0 \lesssim 0.1$.
  \item \textbf{Off-diagonal (hopping) disorder}: Random perturbations to
    the hopping amplitudes $\tau_j \in [-\sigma_\tau, \sigma_\tau]$.
    The protocol maintains high fidelity for $\sigma_\tau/t_0 \lesssim 0.1$.
\end{enumerate}

The CD protocol with different (site-dependent) NNN hoppings is generally
more robust than the equal-hopping version, as the site-dependent parameters
can partially compensate local perturbations. The robustness threshold of
${\sim}\,10\%$ disorder strength relative to $t_0$ is consistent with the
energy scale hierarchy required for the CD correction to remain a perturbative
supplement.
...
\textbf{Domain wall approach} (Zurita \textit{et al.}~\cite{Zurita2023}).
...
\textbf{CD approach} (Romero \textit{et al.}~\cite{Romero2024}).
...
following Zurita~\textit{et al.}~\cite{Zurita2023} and hybrid analog-digital
counterdiabatic driving following Romero~\textit{et al.}~\cite{Romero2024}.
...
\textbf{3.~STA protocols cannot overcome topological constraints.}
...
\emph{completely ineffective} against the coupling asymmetry. This establishes
a clear separation between the two sources of infidelity: (i) diabatic
excitations during the ramp phases, which are addressable by pulse engineering,
and (ii) the coupling asymmetry during the transfer phase, which is a
topological property of the chain and can only be addressed by modifying the
chain structure.
...
corrections. The CD terms, derived from adiabatic gauge potentials via nested
commutators~\cite{Sels2017}, can be further optimized using variational quantum
circuits. In the nonadiabatic regime ($T = \pi/\Omega$, a $100\times$
...
and stringent time constraints. A promising future direction is to combine both
strategies---using domain wall-mediated transfer as the primary mechanism while
adding CD corrections to the preparation and readout phases---leveraging
topological protection for the transfer dynamics and counterdiabatic driving for
the time-critical ramp phases.

\textbf{6.~Parity constraint on domain wall placement.}
We identify that domain walls can only be placed at odd-index sites, and that
a perfectly centered symmetric configuration requires $L = 4k + 3$. This
geometric constraint must be accounted for in experimental designs.

These findings provide a comprehensive picture of topological quantum state
transfer in SSH chains. The exponential speed-up
predicted for multi-domain ($N > 2$) chains~\cite{Zurita2023}, the variational
optimization strategies for CD driving~\cite{Romero2024, Hegade2021}, and the
